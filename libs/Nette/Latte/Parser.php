...
class NParser extends NObject
{
	...
	public function parse($input)
	{
		...
		while ($this->offset < strlen($input)) {
			...
			if (!$matches) { // EOF
				...
				$token = $this->addToken(NLatteToken::MACRO, $matches[0]);
	...
	private function contextText()
	{
		...
		if (!empty($matches['htmlcomment'])) { // <!--
			$this->addToken(NLatteToken::TAG_BEGIN, $matches[0]);
			$this->setContext(self::CONTEXT_COMMENT);

		} elseif (!empty($matches['tag'])) { // <tag or </tag
			$token = $this->addToken(NLatteToken::TAG_BEGIN, $matches[0]);
	...
	private function contextTag()
	{
		...
		if (!empty($matches['end'])) { // end of HTML tag />
			$this->addToken(NLatteToken::TAG_END, $matches[0]);
			...
			$token = $this->addToken(NLatteToken::ATTRIBUTE, $matches[0]);
	...
	protected function filter()
	{
		...
		if ($token->type === NLatteToken::MACRO && $token->name === '/syntax') {
			$this->setSyntax($this->defaultSyntax);
			$token->type = NLatteToken::COMMENT;

		} elseif ($token->type === NLatteToken::MACRO && $token->name === 'syntax') {
			$this->setSyntax($token->value);
			$token->type = NLatteToken::COMMENT;

		} elseif ($token->type === NLatteToken::ATTRIBUTE && $token->name === 'n:syntax') {
			$this->setSyntax($token->value);
			$this->endTag = '/' . $this->lastTag;
			$token->type = NLatteToken::COMMENT;

		} elseif ($token->type === NLatteToken::TAG_END && $this->lastTag === $this->endTag) {
			$this->setSyntax($this->defaultSyntax);

		} elseif ($token->type === NLatteToken::MACRO && $token->name === 'contentType') {
