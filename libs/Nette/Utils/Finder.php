...
class NFinder extends NObject implements IteratorAggregate
{
	...
	private function buildIterator($path)
	{
		...
		if ($this->exclude) {
			...
			$iterator = new NNRecursiveCallbackFilterIterator($iterator, create_function('$file', 'extract(NCFix::$vars['.NCFix::uses(array('filters'=>$filters)).'], EXTR_REFS);
		...
		if ($this->groups) {
			$groups = $this->groups;
			$iterator = new NNCallbackFilterIterator($iterator, create_function('$file', 'extract(NCFix::$vars['.NCFix::uses(array('groups'=>$groups)).'], EXTR_REFS);
