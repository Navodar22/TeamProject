 ...
 * @property-read SystemContainer|IDIContainer $context
...
abstract class NPresenter extends NControl implements IPresenter
{
	...
	private static function argsToParams($class, $method, & $args, $supplemental = array())
	{
		...
		foreach ($rm->getParameters() as $param) {
			...
			if ($val === NULL) {
				...
				if (!is_array($val)) {
					throw new NInvalidLinkException("Invalid value for parameter '$name', expected array.");
				}
			} elseif ($param->getClass() || is_object($val)) {
				// ignore
			} elseif (!is_scalar($val)) {
				throw new NInvalidLinkException("Invalid value for parameter '$name', expected scalar.");
				...
				if ((string) $args[$name] !== (string) $val) {
					throw new NInvalidLinkException("Invalid value for parameter '$name', expected ".gettype($def).".");
				}
			}
	...
	/**
	 * Gets the context.
	 * @return SystemContainer|IDIContainer
	 */
