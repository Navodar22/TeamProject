...
{	
	...
	public function setDataSource($dataSource) {
		$this->dataSource = new GridDataSource($dataSource);
	}
	...
	public function addColumn($column_name, $caption) {
		...
		if(in_array($column_name, $this->dataSource->exist_columns)) {
			if(!array_key_exists($column_name, $this->th)) {
				$this->th[$column_name] = new TextColumn($this, $caption);
			} else {
	...
	public function addBoolColumn($column_name, $caption) {
		if(in_array($column_name, $this->dataSource->exist_columns)) {
			if(!array_key_exists($column_name, $this->th)) {
				$this->th[$column_name] = new BoolColumn($this, $caption);
			} else {
				throw new Exception("Two columns have the same source name - *$column_name*");
			}
		} else {
	...
	public function addDateColumn($column_name, $caption) {
		if(in_array($column_name, $this->dataSource->exist_columns)) {
			if(!array_key_exists($column_name, $this->th)) {
				$this->th[$column_name] = new DateColumn($this, $caption);
			} else {
				throw new Exception("Two columns have the same source name - *$column_name*");
			}
		} else {
	...
	public function addCustomColumn($column_name, $caption) {
		if(!array_key_exists($column_name, $this->th)) {
			$this->th[$column_name] = new CustomColumn($this, $caption);
		} else {
			throw new Exception("Two columns have the same source name - *$column_name*");
		}
	...
	public function formSubmit($form) {
		...
		foreach($form->values as $key => $value) {				
			$value = NStrings::normalize($value);
			if(!empty($value)) {
				$set_values[$key] = array(
					'value' => $this->th[$key]->filter == 'bool' ? $this->stringToBool($value) : $value,
					'kind' => $this->th[$key]->filter,
				);
			}
		}

		$this->filter = $set_values;
		$this->page = '1';
		$this->finalize();
	}
	...
	public function createComponentPagingForm() {
		...
		$self = $this;
		$form->onSuccess[] = callback($this, 'pagingFormSubmit');
	...
	public function pagingFormSubmit($form) {
		$this->handlePage($form->values->page);
	}
	...
	public function createComponentDropdownForm() {
		...
		$self = $this;
		$form->onSuccess[] = callback($this, 'dropdownFormSubmit');
	...
	public function dropdownFormSubmit($form) {
		$this->itemsPerPage = $form->values->itemsPerPage;
			
		if(isSet($this->filter)) {
			foreach($this->filter as $key => $value) { 
				if($value['kind'] == 'bool') {
					$this->filter[$key]['value'] = $this->stringToBool($value['value']);
				}			
			}
		}
	...
	public function getSession() {
		$this->gridSession =  NEnvironment::getSession('Component/Grid/' . $this->name);
	}
